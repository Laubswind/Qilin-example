...
contract allstate_parameters{
    ...
    //更改保证金种类
    function update_margintypes(address[] memory token) external {
        require(msg.sender == factory, 'Qilin: FORBIDDEN');
        Margin_tpyes = token;
    }
    ...
    //查询保证金种类
    function Get_margintypes() external view returns (address[] memory token){
